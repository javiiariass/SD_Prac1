...
- He hecho una variante de la función `Comprobar` que nos has proporcionado. Esta función también devuelve verdadero o falso, pero modifica la Cadena pasada por parámetro para:
	- Si encuentra el texto en el campo de libro -> lo formateamos resaltandolo en negrita. Devuelve *true*
	- Si no encuentra el texto lo deja igual devolviendo *false*.
...
No se indica, en el caso de que el vector no tenga libros, si devolver *TRUE* o *FALSE*
En caso de que no haya libros, no ordeno el vector pero devuelvo *TRUE* (el vector vacío está ordenado).
